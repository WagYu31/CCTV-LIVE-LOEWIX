  ...
  <script>
    ...
    function isValidHumanFaceLandmarks(landmarks, box, isCCTV = false) {
      if (box) {
        ...
        const aspect = bh / bw;
        // Human faces are upright ovals (aspect ratio 0.60 - 2.4). Flat objects like car grills (aspect < 0.5) are rejected.
        if (aspect < 0.60 || aspect > 2.4) return false;
      }
      if (!landmarks) return !isCCTV;
    ...
    // High-Precision Real-Time Webcam Face & Head Tracker (YCbCr Chrominance Segmentation)
    function findLiveWebcamHumanFace(canvas) {
      ...
      try {
        ...
        // Search primary head area: upper 80% of frame, central 75% width
        const startY = Math.round(h * 0.08);
        const endY = Math.round(h * 0.82);
        const startX = Math.round(w * 0.12);
        const endX = Math.round(w * 0.88);
        const step = 4;
        ...
        for (let y = startY; y < endY; y += step) {
          for (let x = startX; x < endX; x += step) {
            ...
            // Standard YCbCr human skin chrominance test
            // Rejects red gaming chairs, black hoods, bright backgrounds, blue clothing
            const Y  =  0.299 * r + 0.587 * g + 0.114 * b;
            const Cb = -0.1687 * r - 0.3313 * g + 0.5 * b + 128;
            const Cr =  0.5 * r - 0.4187 * g - 0.0813 * b + 128;

            if (Y >= 20 && Cb >= 70 && Cb <= 140 && Cr >= 125 && Cr <= 180) {
              sumX += x;
              sumY += y;
              count++;
              if (x < minX) minX = x;
        ...
        // Need at least 15 skin samples to form a valid human face
        if (count >= 15) {
          ...
          const fw = Math.max(65, Math.min(Math.round(w * 0.36), Math.round(spanW * 0.88)));
          const fh = Math.round(fw * 1.25); // human face golden ratio

          const bx = Math.max(4, Math.min(w - fw - 4, Math.round(cx - fw / 2)));
          const by = Math.max(4, Math.min(h - fh - 4, Math.round(cy - fh * 0.42)));
    ...
    async function runFaceAPIDetection(videoElem, providedCanvas = null) {
      ...
      try {
        ...
        const tinyScoreThreshold = isCCTVMode ? 0.35 : 0.12;

        let detections = [];

        // 1. FAST REAL-TIME PRIMARY ENGINE: face-api.js TinyFaceDetector (Ultra responsive on webcam: 0.12)
    ...
    function startAIHUDLoop() {
      ...
      function loop() {
        ...
        if (isAutoTrackingActive) {
          if (lastFaceAPIResult && lastFaceAPIResult.faces.length > 0 && (now - lastFaceAPIResult.timestamp < 3500)) {
            ...
            // Smooth zero-flicker persistence: keep active tracked entities alive for up to 2500ms
            activeAIEntities = activeAIEntities.filter(e => now - (e.createdAt || now) < 2500);
          }
        } else {
