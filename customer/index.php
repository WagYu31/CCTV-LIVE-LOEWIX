...
<body>
  ...
  <script>
    ...
    function openLivePlayerModal(camId) {
      const cam = customerCameras.find(c => c.id == camId);
      if (!cam) return;

      document.getElementById('playerModalTitle').innerHTML = `<i class="fas fa-video text-info mr-2"></i> ${cam.title}`;
      document.getElementById('playerModalSubtitle').innerText = `Lokasi: ${(cam.city || 'Siantar').toUpperCase()} | Tipe: ${cam.connection_type || 'RTSP'}`;

      const video = document.getElementById('customer-preview-video');
      const streamUrl = cam.hls_url || cam.streamPath;
