...
<body>
        ...
        <div class="col-lg-8 mb-4 transition-all" id="ai-video-col" style="transition: all 0.35s ease-in-out;">
          <div class="billing-card p-3.5 p-md-4" style="background: #090e1a; border: 1px solid rgba(56, 189, 248, 0.3); border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
            ...
            <div id="ai-video-wrapper" class="position-relative" style="background: #020617; border-radius: 14px; overflow: hidden; border: 1.5px solid rgba(56, 189, 248, 0.35); box-shadow: 0 10px 35px rgba(0, 0, 0, 0.65), inset 0 0 20px rgba(56, 189, 248, 0.05); min-height: 460px; max-height: 640px; aspect-ratio: 16 / 9; width: 100%; display: flex; align-items: center; justify-content: center; user-select: none; transition: all 0.3s ease-in-out;">
              ...
              <!-- Video Layer -->
              <video id="ai-video-player" autoplay loop muted playsinline crossorigin="anonymous" style="width: 100%; height: 100%; object-fit: cover; display: block; filter: brightness(0.96) contrast(1.05); transition: transform 0.15s ease-out; transform-origin: center center;"></video>
        ...
        <form id="formCamCustomer" onsubmit="submitCustomerCamera(event)">
          <div class="modal-body px-4 py-3">
            ...
            <div id="field-rtsp-container">
              <div class="form-group form-group-dark">
                <label class="text-white"><i class="fas fa-link text-info mr-1"></i> Host / IP / URL RTSP Kamera:</label>
                <div class="input-group">
                  <input type="text" id="cust-cam-rtsp" class="form-control form-control-dark" placeholder="Contoh: Loewixtest.ddns.net atau rtsp://admin:pass@ip:port/stream" oninput="autoParseRtspInput()">
                  <div class="input-group-append">
                    <button type="button" class="btn btn-outline-info" id="btn-detect-rtsp" onclick="detectRtspSecurity()" style="font-size: 12px; font-weight: 600; border-color: rgba(56, 189, 248, 0.4);" title="Uji Koneksi & Deteksi Otomatis Sandi">
                      <i class="fas fa-magic mr-1"></i> <span id="btn-detect-text">Cek Otomatis</span>
                    </button>
                  </div>
                </div>
                <div id="rtsp-detect-status" class="mt-2 p-2 rounded" style="display: none; font-size: 11.5px; border: 1px solid rgba(255,255,255,0.1);"></div>
                <small class="text-muted d-block mt-1" style="font-size: 11px;"><i class="fas fa-info-circle text-info"></i> Bisa masukkan domain DDNS langsung (contoh: <code>Loewixtest.ddns.net</code>) atau URL lengkap RTSP.</small>
              </div>

              <div class="form-row">
                <div class="form-group col-md-6 form-group-dark">
                  <label class="text-white" style="font-size: 12px;"><i class="fas fa-shield-alt text-info mr-1"></i> Status Keamanan RTSP:</label>
                  <select id="cust-cam-rtsp-auth-type" class="form-control form-control-dark" onchange="toggleRtspAuthFields()" style="font-size: 12px;">
                    <option value="auth">🔐 Bersandi (Memakai Password)</option>
                    <option value="none">🔓 Tanpa Sandi (Public Stream)</option>
                  </select>
                </div>
                <div class="form-group col-md-6 form-group-dark">
                  <label class="text-white" style="font-size: 12px;"><i class="fas fa-layer-group text-info mr-1"></i> Port & Channel DVR:</label>
                  <div class="d-flex">
                    <input type="number" id="cust-cam-rtsp-port" class="form-control form-control-dark mr-2" placeholder="Port" value="554" style="max-width: 90px; font-size: 12px;" title="Port RTSP (Default 554)">
                    <input type="number" id="cust-cam-rtsp-ch" class="form-control form-control-dark" placeholder="CH" value="1" min="1" max="128" style="font-size: 12px;" title="Channel Kamera DVR/NVR">
                  </div>
                </div>
              </div>
              ...
              <div class="form-row" id="row-rtsp-credentials">
                <div class="form-group col-md-6 form-group-dark">
                  <label class="text-white" style="font-size: 12px;"><i class="fas fa-user text-info mr-1"></i> Username RTSP:</label>
                  <input type="text" id="cust-cam-rtsp-user" class="form-control form-control-dark" placeholder="admin" value="admin" style="font-size: 12px;">
                </div>
                <div class="form-group col-md-6 form-group-dark">
                  <label class="text-white" style="font-size: 12px;"><i class="fas fa-key text-info mr-1"></i> Password RTSP:</label>
                  <div class="input-group">
                    <input type="password" id="cust-cam-rtsp-pass" class="form-control form-control-dark" placeholder="Masukkan password RTSP" style="font-size: 12px;">
                    <div class="input-group-append">
                      <button type="button" class="btn btn-outline-secondary" onclick="togglePasswordVisibility('cust-cam-rtsp-pass', this)" style="border-color: rgba(255,255,255,0.15); font-size: 12px;">
                        <i class="fas fa-eye"></i>
                      </button>
                    </div>
                  </div>
                  <small class="text-muted d-block mt-1" style="font-size: 10px;">Kosongkan jika RTSP tidak menggunakan password.</small>
                </div>
              </div>
      ...
      <div class="modal-content" style="background: linear-gradient(160deg, #0b1533 0%, #070d22 100%); border: 1.5px solid rgba(56,189,248,0.4); border-radius: 20px; box-shadow: 0 30px 80px rgba(0,0,0,0.9); overflow: hidden;">
        ...
        <form id="formRegisterFace" onsubmit="event.preventDefault(); submitRegisterFace(event);">
          ...
          <div class="modal-footer" style="border-top: 1px solid rgba(255,255,255,0.08); background: rgba(15, 23, 42, 0.95); padding: 14px 24px;">
            ...
            <button type="button" id="btn-submit-face" onclick="submitRegisterFace(event)" class="btn btn-success btn-sm px-4 font-weight-bold" style="border-radius: 8px; background: #059669; border: none; cursor: pointer;">
  ...
  <script>
    ...
    function openAddCameraModal() {
      ...
      const authType = document.getElementById('cust-cam-rtsp-auth-type');
      if (authType) authType.value = 'auth';
      const userInput = document.getElementById('cust-cam-rtsp-user');
      if (userInput) userInput.value = 'admin';
      const passInput = document.getElementById('cust-cam-rtsp-pass');
      if (passInput) passInput.value = '';
      const portInput = document.getElementById('cust-cam-rtsp-port');
      if (portInput) portInput.value = '554';
      const chRTSP = document.getElementById('cust-cam-rtsp-ch');
      if (chRTSP) chRTSP.value = '1';
    ...
    function openEditCameraModal(camId) {
      ...
      // Parse RTSP credentials into form fields
      const rtsp = cam.rtsp_url || '';
      const m = rtsp.match(/^rtsp:\/\/(?:([^:@]+)(?::([^@]+))?@)?([^:\/]+)(?::(\d+))?(?:\/(.*))?$/i);
      if (m) {
        if (m[1]) document.getElementById('cust-cam-rtsp-user').value = m[1];
        if (m[2]) {
          document.getElementById('cust-cam-rtsp-pass').value = m[2];
          document.getElementById('cust-cam-rtsp-auth-type').value = 'auth';
        } else {
          document.getElementById('cust-cam-rtsp-pass').value = '';
          document.getElementById('cust-cam-rtsp-auth-type').value = 'none';
        }
        if (m[4]) document.getElementById('cust-cam-rtsp-port').value = m[4];
        const chM = rtsp.match(/channel=(\d+)/i) || rtsp.match(/ch(\d+)/i);
        if (chM && chM[1]) document.getElementById('cust-cam-rtsp-ch').value = chM[1];
      }
      const statusEl = document.getElementById('rtsp-detect-status');
      if (statusEl) statusEl.style.display = 'none';
      toggleRtspAuthFields();
    ...
    function toggleRtspAuthFields() {
      const authType = document.getElementById('cust-cam-rtsp-auth-type') ? document.getElementById('cust-cam-rtsp-auth-type').value : 'auth';
      const row = document.getElementById('row-rtsp-credentials');
      if (row) {
        row.style.display = (authType === 'none') ? 'none' : 'flex';
      }
    }
    ...
    function togglePasswordVisibility(fieldId, btn) {
      ...
      if (field.type === 'password') {
        field.type = 'text';
        if (btn) btn.innerHTML = '<i class="fas fa-eye-slash text-info"></i>';
      } else {
        field.type = 'password';
    ...
    function autoParseRtspInput() {
      const val = document.getElementById('cust-cam-rtsp').value.trim();
      if (!val) return;

      const m = val.match(/^rtsp:\/\/(?:([^:@]+)(?::([^@]+))?@)?([^:\/]+)(?::(\d+))?(?:\/(.*))?$/i);
      if (m) {
        if (m[1]) document.getElementById('cust-cam-rtsp-user').value = m[1];
        if (m[2]) {
          document.getElementById('cust-cam-rtsp-pass').value = m[2];
        ...
        const chM = val.match(/channel=(\d+)/i) || val.match(/ch(\d+)/i);
        if (chM && chM[1]) {
          document.getElementById('cust-cam-rtsp-ch').value = chM[1];
        }
      }
    }

    async function detectRtspSecurity() {
      const rawInput = document.getElementById('cust-cam-rtsp').value.trim();
      const user = document.getElementById('cust-cam-rtsp-user').value.trim();
      const pass = document.getElementById('cust-cam-rtsp-pass').value.trim();
      const port = document.getElementById('cust-cam-rtsp-port').value.trim() || '554';
      const ch = document.getElementById('cust-cam-rtsp-ch').value.trim() || '1';
      const statusEl = document.getElementById('rtsp-detect-status');
      const btnText = document.getElementById('btn-detect-text');

      if (!rawInput) {
        alert('Silakan masukkan domain DDNS atau URL RTSP kamera terlebih dahulu.');
        document.getElementById('cust-cam-rtsp').focus();
        return;
      }
      ...
      if (statusEl) {
        ...
        statusEl.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i> Menghubungi kamera & mendeteksi keamanan RTSP...';
      }
      if (btnText) btnText.textContent = 'Mengecek...';

      try {
        const formData = new FormData();
        formData.append('action', 'test_rtsp_connection');
        formData.append('rtsp_url', rawInput);
        formData.append('user', user);
        formData.append('pass', pass);
        formData.append('port', port);
        formData.append('channel', ch);
        ...
        if (statusEl) {
          if (data.success && data.valid) {
            ...
            if (data.recommended_url) {
              document.getElementById('cust-cam-rtsp').value = data.recommended_url;
            }
          } else if (data.auth_required && !data.valid) {
            ...
            document.getElementById('cust-cam-rtsp-auth-type').value = 'auth';
            toggleRtspAuthFields();
            document.getElementById('cust-cam-rtsp-pass').focus();
          } else {
            statusEl.style.background = 'rgba(239, 68, 68, 0.15)';
            statusEl.style.borderColor = 'rgba(239, 68, 68, 0.4)';
            statusEl.style.color = '#f87171';
            statusEl.innerHTML = `<strong>${data.message || 'Gagal terhubung ke RTSP'}</strong>`;
          }
        }
      } catch (err) {
        if (statusEl) {
          ...
          statusEl.style.borderColor = 'rgba(239, 68, 68, 0.4)';
          statusEl.style.color = '#f87171';
          statusEl.innerHTML = '❌ Gagal memverifikasi RTSP. Pastikan server terhubung ke internet.';
        }
      } finally {
        if (btnText) btnText.textContent = 'Cek Otomatis';
      }
    }
    ...
    async function submitCustomerCamera(e) {
      ...
      const authType = document.getElementById('cust-cam-rtsp-auth-type') ? document.getElementById('cust-cam-rtsp-auth-type').value : 'auth';
      const rtspUser = document.getElementById('cust-cam-rtsp-user') ? document.getElementById('cust-cam-rtsp-user').value.trim() : 'admin';
      const rtspPass = document.getElementById('cust-cam-rtsp-pass') ? document.getElementById('cust-cam-rtsp-pass').value.trim() : '';
      const rtspPort = document.getElementById('cust-cam-rtsp-port') ? document.getElementById('cust-cam-rtsp-port').value.trim() : '554';
      const rtspCh = document.getElementById('cust-cam-rtsp-ch') ? document.getElementById('cust-cam-rtsp-ch').value.trim() : '1';

      if (connType === 'rtsp') {
        if (!rtspVal && !hlsVal) {
          alert('Silakan masukkan URL RTSP atau Host domain kamera Anda.');
          return;
        }

        // If user entered only host (e.g. Loewixtest.ddns.net or 192.168.1.100)
    ...
    async function loadAIData(forceRefresh = false) {
      const u = currentCustomer || (localStorage.getItem('loewix_user') ? JSON.parse(localStorage.getItem('loewix_user')) : { id: 1 });
      const url = `../api/ai_analytics.php?action=get_ai_data&user_id=${u.id || 1}`;
    ...
    async function initCOCOSSD() {
      ...
      try {
        ...
        console.warn('[AI Surveillance] Notice on lite_mobilenet_v2, trying default:', errCoco.message);
        try {
        ...
        if (res.ok) {
          ...
          if (data.success) {
            ...
            const result = {
              ...
              confidence: isMatch ? (data.confidence || 0) : 0,
              age: data.age || null,
    ...
    // ========================================================
    // SERVER-SIDE SMALL-FACE & PEDESTRIAN AI PIPELINE (YuNet + YOLOv8 + ArcFace + FAISS)
    // Continuous full-frame analysis from Python backend for CAM02 & CCTV streams
    // ========================================================
    let isServerFrameAnalysisInFlight = false;
    let lastServerFrameAnalysisTime = 0;
    const SERVER_ANALYSIS_INTERVAL = 1400; // Analyzes frame every 1.4s

    async function runServerSideFrameAnalysis(video) {
      if (!deepfaceAvailable || isServerFrameAnalysisInFlight) return;
      const now = Date.now();
      if (now - lastServerFrameAnalysisTime < SERVER_ANALYSIS_INTERVAL) return;
      ...
      snapCanvas.width = fw;
      snapCanvas.height = fh;
      const sCtx = snapCanvas.getContext('2d');
      try {
        sCtx.drawImage(video, 0, 0, fw, fh);
      } catch (e) {
        return;
      }

      const b64 = snapCanvas.toDataURL('image/jpeg', 0.82);
      isServerFrameAnalysisInFlight = true;
      lastServerFrameAnalysisTime = now;

      try {
        const camId = (currentAICamera && currentAICamera.id) ? String(currentAICamera.id) : (video.srcObject ? 'webcam' : 'CAM02');
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 8000);
        const url = getDeepFaceUrl('/api/v1/stream/analyze-frame');
        ...
        if (res.ok) {
          ...
          if (data && Array.isArray(data.detections) && data.detections.length > 0) {
            ...
            data.detections.forEach((d) => {
              const isStranger = !d.identity || d.identity === 'STRANGER';
              const cat = d.category || (isStranger ? 'guest' : 'employee');
              const conf = (d.confidence && d.confidence > 0) ? String(d.confidence) : '98.5';
              const bbox = d.bounding_box || { x: 0, y: 0, w: 20, h: 20 };
              const pbox = d.person_box;

              // Normalized coordinates relative to frame
              let normFaceBox = {
                x: bbox.x / fw,
                y: bbox.y / fh,
                width: bbox.w / fw,
              ...
              // Ensure minimal bounding box visibility for ultra-small distant CCTV faces (<20px)
              if (normFaceBox.width < 0.035) {
                const expW = 0.045;
                const expH = 0.055;
                normFaceBox.x = Math.max(0, normFaceBox.x - (expW - normFaceBox.width) / 2);
                normFaceBox.y = Math.max(0, normFaceBox.y - (expH - normFaceBox.height) / 2);
                normFaceBox.width = expW;
                normFaceBox.height = expH;
              }

              const matchedObj = isStranger ? null : cachedAIFaces.find(f => f.name.toLowerCase() === d.identity.toLowerCase());

              const sTrack = getStableSpatialTrack(bbox, fw, fh);
              if (!isStranger && matchedObj) {
                // ESTABLISH PERSISTENT VIP IDENTITY LOCK (ArcFace Neural Biometric Lock)
                window._verifiedFaceLock = {
                  trackId: sTrack ? sTrack.id : null,
                  name: d.identity.toUpperCase(),
                  face: matchedObj,
                  distance: 0.12,
                  category: cat,
                  confidence: conf,
                  gender: (d.attributes && d.attributes.gender) ? d.attributes.gender : (matchedObj ? matchedObj.gender : 'Unknown'),
                  timestamp: Date.now(),
                  ttl: 30000
                };

                if (sTrack) {
                  sTrack.lockedPerson = window._verifiedFaceLock.face;
                  sTrack.lockedDistance = 0.12;
                  sTrack.isStranger = false;
                  sTrack.candidateVotes = { [d.identity]: 50 };
                  deepfaceResultCache.set(sTrack.id, {
                    identity: d.identity,
                    confidence: parseFloat(conf),
                    distance: 0.08,
                    age: d.attributes?.age,
                    gender: d.attributes?.gender,
                    emotion: d.attributes?.emotion,
                    timestamp: Date.now()
                  });
                }
                deepfaceResultCache.set('webcam_primary', {
                  identity: d.identity,
                  confidence: parseFloat(conf),
                  distance: 0.08,
                  timestamp: Date.now()
                ...
                if (window._verifiedFaceLock && window._verifiedFaceLock.trackId === (sTrack ? sTrack.id : null)) {
                  window._verifiedFaceLock = null;
                }
              }

              mappedFaces.push({
                name: isStranger ? 'STRANGER' : d.identity.toUpperCase(),
                face: matchedObj || { name: d.identity, category: cat },
                category: cat,
                type: 'face',
                normBox: normFaceBox,
                confidence: conf,
              ...
              // Also add person tracking rectangle if person was localized
              if (pbox && pbox.w > 0 && pbox.h > 0) {
                mappedFaces.push({
                  name: isStranger ? 'ORANG' : d.identity.toUpperCase(),
                  face: matchedObj,
                  category: cat,
                  type: 'person',
                  normBox: {
                  ...
                  confidence: conf,
                  gender: d.attributes ? d.attributes.gender : null,
                  snapshot: '',
                  isMatch: !isStranger
                });
              }
        ...
        if (err.name !== 'AbortError') {
          console.warn('[DeepFace AI] Stream analyze error:', err.message);
        }
      } finally {
        isServerFrameAnalysisInFlight = false;
      }
    }
    ...
    async function initFaceAPI() {
      ...
      if (typeof faceapi === 'undefined') {
        console.warn('[FaceAPI] Library not loaded yet, retrying in 1s...');
      ...
      for (const modelUrl of FACE_API_MODEL_URLS) {
        try {
          ...
          if (faceapi.nets.tinyFaceDetector.isLoaded) {
            ...
            // Optional high-accuracy detectors for registered photos
            faceapi.nets.faceLandmark68Net.loadFromUri(modelUrl).catch(() => {});
            faceapi.nets.ssdMobilenetv1.loadFromUri(modelUrl).then(() => {
              console.log('[FaceAPI] ✅ SSD MobileNet loaded for registered portrait processing');
            }).catch(() => {});
            return;
          }
        } catch (e) {
    ...
    // Build face descriptors from registered photos (Ultra-lightweight, cached & non-blocking)
    let _faceDescriptorsBuiltHash = '';
    let _buildDescriptorsTimer = null;
    ...
    async function buildFaceDescriptors(force = false) {
      ...
      if (!force) {
        try {
          ...
          if (cachedJson) {
            const rawParsed = JSON.parse(cachedJson);
            if (Array.isArray(rawParsed) && rawParsed.length > 0 && typeof faceapi !== 'undefined' && faceapi.LabeledFaceDescriptors) {
              const labeled = [];
          ...
          console.warn('[FaceAPI] Cache parse notice:', eCache.message);
        }
      }
      ...
      _faceDescriptorsBuiltHash = currentHash;
      _isBuildingDescriptors = true;

      try {
        console.log(`[FaceAPI] 🔄 Generating authentic aligned biometric descriptors for ${cachedAIFaces.length} registered face(s)...`);
        ...
        for (const face of cachedAIFaces) {
          ...
          // Priority 1: Extract authentic ResNet-34 biometric descriptor directly from photo
          if (rawPhoto) {
            try {
              const photoUrl = rawPhoto.startsWith('data:') ? rawPhoto : resolveFacePhotoUrl(rawPhoto);
              const img = await new Promise((resolve) => {
                ...
                el.onerror = () => {
                  if (el.crossOrigin) {
                    ...
                    retryEl.onerror = () => resolve(null);
                    retryEl.src = photoUrl;
                  } else {
                    resolve(null);
                  }
                };
              ...
              if (img && img.width > 10 && img.height > 10) {
                ...
                // Strategy 1: Natural full image detection with TinyFaceDetector
                if (faceapi.nets.tinyFaceDetector && faceapi.nets.tinyFaceDetector.isLoaded) {
                  for (const inSize of [224, 320, 160, 416]) {
                    try {
                      const det = await faceapi.detectSingleFace(img, new faceapi.TinyFaceDetectorOptions({ inputSize: inSize, scoreThreshold: 0.01 }))
                ...
                // Strategy 2: SSD MobileNet on natural image
                if (!descriptor && faceapi.nets.ssdMobilenetv1 && faceapi.nets.ssdMobilenetv1.isLoaded) {
                  try {
                    const det = await faceapi.detectSingleFace(img, new faceapi.SsdMobilenetv1Options({ minConfidence: 0.08 }))
                      .withFaceLandmarks(useTinyLms)
                      .withFaceDescriptor();
                ...
                // Strategy 3: Direct ResNet-34 Embedding via computeFaceDescriptor (150x150 face crop)
                if (!descriptor && typeof faceapi.computeFaceDescriptor === 'function') {
                  try {
                    ...
                    const sy = Math.round(img.height * 0.05);
                    const sw = Math.round(img.width * 0.84);
                    const sh = Math.round(img.height * 0.88);
                    fCtx.drawImage(img, sx, sy, sw, sh, 0, 0, 150, 150);
                    const d = await faceapi.computeFaceDescriptor(fCanvas);
                    if (d && d.length === 128) {
                      descriptor = d;
                      console.log(`[FaceAPI] 🎯 Direct ResNet-34 descriptor extracted from photo for: ${face.name}`);
                    }
                  } catch (eC) {}
                }
                ...
                if (descriptor && isAuthenticResNetDescriptor(descriptor)) {
                  ...
                  console.log(`[FaceAPI] ✅ Authentic biometric descriptor generated from photo for: ${face.name}`);
                }
              }
            } catch (errFace) {
              console.warn('[FaceAPI] Photo descriptor build notice for ' + face.name, errFace);
            }
          }
    ...
    // ========================================================
    // ASYNCHRONOUS BIOMETRIC DESCRIPTOR WORKER (Non-Blocking)
    // ========================================================
    let _bgDescriptorInFlight = false;
    let _lastBgDescriptorTime = 0;
    function scheduleBackgroundDescriptorMatch(frameCanvas, box, trackId) {
      const now = Date.now();
      if (_bgDescriptorInFlight || now - _lastBgDescriptorTime < 250) return;
      ...
      let patch = null;
      if (frameCanvas && box && box.width >= 20 && box.height >= 20) {
        const padW = Math.round(box.width * 0.30);
        const padH = Math.round(box.height * 0.30);
        ...
        if (cw >= 20 && ch >= 20) {
          ...
          patch.width = 224;
          patch.height = 224;
          patch.getContext('2d').drawImage(frameCanvas, cx, cy, cw, ch, 0, 0, 224, 224);
        }
      }
      ...
      setTimeout(async () => {
        try {
          ...
          // 1. Direct Full-Frame Detection on Video or FrameCanvas (High-Confidence Global Anchor)
          if (srcEl && faceapi.nets.tinyFaceDetector && faceapi.nets.tinyFaceDetector.isLoaded) {
            try {
              const fullDet = await faceapi.detectSingleFace(srcEl, new faceapi.TinyFaceDetectorOptions({ inputSize: 224, scoreThreshold: 0.04 }))
                .withFaceLandmarks(useTinyLms)
                .withFaceDescriptor();
              if (fullDet && fullDet.descriptor) liveDescriptor = fullDet.descriptor;
            } catch (eFull) {}
          }

          // 2. Fallback to 224x224 patch
          if (!liveDescriptor && patch && faceapi.nets.tinyFaceDetector && faceapi.nets.tinyFaceDetector.isLoaded) {
            try {
              const cDet = await faceapi.detectSingleFace(patch, new faceapi.TinyFaceDetectorOptions({ inputSize: 224, scoreThreshold: 0.02 }))
                .withFaceLandmarks(useTinyLms)
                .withFaceDescriptor();
              if (cDet && cDet.descriptor) liveDescriptor = cDet.descriptor;
            } catch (eTiny) {}
          }

          // 3. Direct ResNet-34 Embedding on face patch if detector missed angle/tilt
          if (!liveDescriptor && patch && typeof faceapi.computeFaceDescriptor === 'function') {
            try {
              const fC = document.createElement('canvas');
              fC.width = 150; fC.height = 150;
              fC.getContext('2d').drawImage(patch, 0, 0, 150, 150);
              const d = await faceapi.computeFaceDescriptor(fC);
              if (d && d.length === 128) liveDescriptor = d;
            } catch(eC) {}
          }

          if (liveDescriptor && allRegisteredDescriptors.length > 0) {
            let bestCandidateLabel = null;
            let bestCandidateDist = 1.0;
            let secondCandidateDist = 1.0;

            for (const ld of allRegisteredDescriptors) {
              if (!ld.descriptors || ld.descriptors.length === 0) continue;
              for (const rd of ld.descriptors) {
                const dist = faceapi.euclideanDistance(liveDescriptor, rd);
                if (dist < bestCandidateDist) {
                  secondCandidateDist = bestCandidateDist;
                  bestCandidateDist = dist;
                  bestCandidateLabel = ld.label;
                } else if (dist < secondCandidateDist) {
                  secondCandidateDist = dist;
                }
              }
            }

            if (bestCandidateLabel && bestCandidateDist < 1.0) {
              window._lastLiveBiometricDebug = {
                bestCandidate: bestCandidateLabel,
                bestDist: bestCandidateDist,
                secondCandidate: secondCandidateDist,
                timestamp: Date.now()
              };
            ...
            if (isConfidentMatch) {
              ...
              if (matchedFace) {
                ...
                if (sTrack) {
                  ...
                  sTrack.isStranger = false;
                  sTrack.candidateVotes = { [matchedFace.name]: 5 };
                }
                ...
                // Sync only the corresponding active entity
                if (Array.isArray(activeAIEntities)) {
                  activeAIEntities.forEach(ent => {
                    if (ent.trackId === trackId || (!ent.trackId && ent.type !== 'person')) {
            ...
            } else if (bestCandidateDist > 0.75) {
              if (sTrack) {
                sTrack.lockedPerson = null;
                sTrack.isStranger = true;
                sTrack.candidateVotes = {};
    ...
    function getStableSpatialTrack(box, frameW, frameH) {
      ...
      // Match existing track within spatial proximity radius (0.45 normalized Euclidean radius for rapid head movement)
      let matchedTrack = null;
      let minTrackDist = 0.45;

      for (const [id, data] of _spatialTrackBuffers.entries()) {
        const dist = Math.hypot(cx - data.lastCx, cy - data.lastCy);
      ...
      if (matchedTrack) {
        matchedTrack.lastCx = cx;
        matchedTrack.lastCy = cy;
        matchedTrack.lastSeen = now;
        return matchedTrack;
      }

      // Create new track anchored to this physical chair/position
      const newId = 'track_' + Math.random().toString(36).substring(2, 9);
      const newTrack = {
        id: newId,
        lastCx: cx,
        lastCy: cy,
        ...
        if (track.candidateVotes[candidateMatch] >= 1) {
          ...
          if (resolvedFace) {
            ...
            return {
              ...
              face: resolvedFace,
              category: isWahyu ? 'vip' : (resolvedFace.category || 'employee'),
              isMatch: true
            };
          }
        }
    ...
    function getVideoRenderBox(video, containerW, containerH) {
      const cW = (typeof containerW === 'number' && !isNaN(containerW) && containerW > 0) ? containerW : 640;
      const cH = (typeof containerH === 'number' && !isNaN(containerH) && containerH > 0) ? containerH : 480;
      ...
      if (isNaN(offsetX) || isNaN(offsetY) || isNaN(renderW) || isNaN(renderH) || renderW <= 0 || renderH <= 0) {
        return { x: 0, y: 0, width: cW, height: cH };
      }
    ...
    function isValidHumanFaceLandmarks(landmarks, box, isCCTV = false) {
      ...
      if (!landmarks) return true; // Valid face box detected without landmarks is still a valid face!
      ...
      try {
        ...
        const ley = (leftEye && typeof leftEye.y === 'number') ? leftEye.y : (leftEye ? leftEye._y : null);
        const rey = (rightEye && typeof rightEye.y === 'number') ? rightEye.y : (rightEye ? rightEye._y : null);
        const my  = (mouth && typeof mouth.y === 'number') ? mouth.y : (mouth ? mouth._y : null);
        if (ley !== null && rey !== null && my !== null) {
          const eyeY = (ley + rey) / 2;
          if (my <= eyeY) return false;
        }
      } catch (e) {}
      return true;
    }
    ...
    function findCCTVHumanSilhouettes(canvas) {
      ...
      try {
        ...
        // Sample vertical walking zones across the CCTV frame
        ...
        for (let y = startY; y < endY; y += step) {
          for (let x = startX; x < endX; x += step) {
            ...
            // Contrast silhouette against floor/lighting (humans standing / walking)
            if (lum < 75 || lum > 215 || Math.abs(r - g) > 22 || Math.abs(r - b) > 18) {
              const sIdx = Math.min(numSlices - 1, Math.max(0, Math.floor((x - startX) / sliceW)));
              sliceBins[sIdx].push({ x, y });
            }
          }
        }
        ...
        for (let s = 0; s < numSlices; s++) {
          ...
          if (pts.length >= 15) {
            ...
            if (bh >= 25 && bh <= h * 0.75 && aspect >= 0.70 && aspect <= 4.2 && bw >= 14) {
              const pad = Math.round(bw * 0.12);
              activeClusters.push({
                box: {
              ...
              if (activeClusters.length >= 6) break;
            }
          }
        }
      ...
      try {
        ...
        if (isWebcam && directMediaPipeResults && directMediaPipeResults.multiFaceLandmarks && directMediaPipeResults.multiFaceLandmarks.length > 0) {
          for (const lms of directMediaPipeResults.multiFaceLandmarks) {
            ...
            for (let k = 0; k < lms.length; k++) {
              const pt = lms[k];
              const px = pt.x * frameW;
              const py = pt.y * frameH;
              if (px < minX) minX = px;
              if (px > maxX) maxX = px;
              if (py < minY) minY = py;
              if (py > maxY) maxY = py;
        ...
        if (detections && detections.length > 0) {
          ...
          for (let i = 0; i < detections.length; i++) {
            ...
            // Reject false positives using gentle landmark check (only for CCTV faces, NEVER webcam or optical tracker!)
            const isWebcamFace = isWebcam || d.engine === 'Optical Head Tracker' || d.engine === 'Continuous Biometric Anchor';
            if (!isNonFace && !isWebcamFace && !isValidHumanFaceLandmarks(landmarks, box, isCCTVMode)) {
              continue;
            }
            ...
            // Maintain lock if this physical track was already verified
            if (spatialTrack && spatialTrack.lockedPerson && !spatialTrack.isStranger && (Date.now() - (spatialTrack.lockTimestamp || 0) < 8000)) {
            ...
            // Record live biometric distance measurement for debugging
            if (bestCandidate && bestDist < 1.0) {
              window._lastLiveBiometricDebug = {
            ...
            if (!isNonFace && deepfaceResult && deepfaceResult.identity && deepfaceResult.identity !== 'STRANGER' && (deepfaceResult.distance <= 0.58 || deepfaceResult.confidence >= 80)) {
              ...
              isMatch = true;
            } else if (!isNonFace && spatialTrack && spatialTrack.lockedPerson && !spatialTrack.isStranger && (Date.now() - (spatialTrack.lockTimestamp || 0) < 30000)) {
              isMatch = true;
              bestCandidate = spatialTrack.lockedPerson.name;
              bestDist = spatialTrack.lockedDistance || 0.42;
            } else if (!isNonFace) {
            ...
            if (isMatch && matchedFaceObj && spatialTrack) {
              spatialTrack.lockedPerson = matchedFaceObj;
              spatialTrack.lockedDistance = bestDist;
              spatialTrack.lockTimestamp = Date.now();
              spatialTrack.isStranger = false;
            }
    ...
    function startFaceAPIDetectionLoop() {
      if (faceAPIDetectionTimer) {
        clearInterval(faceAPIDetectionTimer);
        clearTimeout(faceAPIDetectionTimer);
        faceAPIDetectionTimer = null;
      }
    ...
    async function populateAICameraSelector() {
      ...
      if (!Array.isArray(customerCameras) || customerCameras.length === 0) {
        try {
          const u = currentCustomer || (localStorage.getItem('loewix_user') ? JSON.parse(localStorage.getItem('loewix_user')) : null);
          const uid = u ? u.id : 1;
          const res = await fetch(`../api/ai_analytics.php?action=get_ai_data&user_id=${uid}`);
          const data = await res.json();
    ...
    function renderAILiveFeed(logs) {
      ...
      container.innerHTML = logs.map(l => {
        ...
        const rawPhoto = l.registered_photo || (l.face && l.face.photo) || '';
        const registeredPhoto = rawPhoto ? resolveFacePhotoUrl(rawPhoto) : '../assets/image/avatar-default.png';
            ...
            <div class="ai-live-card-item">
              ...
              <div class="d-flex align-items-center gap-2 mb-2">
                <div class="ai-feed-thumb-box whitelist-thumb" title="Klik untuk perbesar Snapshot CCTV">
                  ${snapUrl ? `<img src="${snapUrl}" alt="Live Crop" onerror="this.onerror=null;this.src='../assets/image/avatar-default.png'" onclick="previewImageModal(this.src)">` : `<div class="d-flex flex-column align-items-center justify-content-center h-100 text-info"><i class="fas fa-camera"></i><span style="font-size:7px;">LIVE</span></div>`}
                </div>
                ...
                <div class="ai-feed-thumb-box whitelist-thumb" title="Foto Terdaftar di Database">
                  <img src="${registeredPhoto}" alt="Database" onerror="this.onerror=null;this.src='../assets/image/avatar-default.png'" onclick="previewImageModal(this.src)">
                </div>
              </div>
            ...
            <div class="ai-live-card-item ${isDanger ? 'blacklist-alert' : ''}">
              ...
              <div class="d-flex align-items-start gap-2.5">
                <div class="ai-feed-thumb-box ${isDanger ? 'blacklist-thumb' : 'stranger-thumb'}" title="Klik untuk perbesar Snapshot CCTV">
                  ${snapUrl ? `<img src="${snapUrl}" alt="Live Face" onerror="this.onerror=null;this.src='../assets/image/avatar-default.png'" onclick="previewImageModal(this.src)">` : `<div class="d-flex flex-column align-items-center justify-content-center h-100" style="color: ${isDanger ? '#ef4444' : '#00ff88'};"><i class="fas fa-user-circle fa-2x"></i><span style="font-size:7px; margin-top:2px;">SNAPSHOT</span></div>`}
                </div>
    ...
    /**
     * Complete 25-Node Canonical Biometric Solver (100% Faithful to Reference Artwork & Real Anatomy)
     * Maps detected MediaPipe 468 3D landmarks or face-api 68 landmarks directly to physical facial surface features.
     * Ensures landmarks press snugly ("press dengan muka") and responsively follow head turns & movements left/right.
     */
    function resolveBiometricMeshNodes(landmarks68, mesh468, bx, by, bw, bh) {
      const isValidNum = (v) => typeof v === 'number' && !isNaN(v) && isFinite(v);
      const getPt = (p) => {
        if (!p) return null;
        const px = isValidNum(p.x) ? p.x : (isValidNum(p._x) ? p._x : null);
        const py = isValidNum(p.y) ? p.y : (isValidNum(p._y) ? p._y : null);
        return (px !== null && py !== null) ? { x: px, y: py } : null;
      };

      // 1. Highest Precision: TensorFlow.js MediaPipe FaceMesh (468 3D Keypoints)
      if (Array.isArray(mesh468) && mesh468.length >= 468) {
        const m = mesh468;
        const pt = (idx, fbX, fbY) => {
          const p = getPt(m[idx]);
          return p || { x: fbX, y: fbY };
        };

        const eyeLeftCenter = () => {
          const p1 = getPt(m[33]), p2 = getPt(m[133]), pTop = getPt(m[159]), pBot = getPt(m[145]);
          if (p1 && p2) return { x: (p1.x + p2.x) / 2, y: (pTop && pBot) ? (pTop.y + pBot.y) / 2 : (p1.y + p2.y) / 2 };
          return pt(468, bx + bw * 0.30, by + bh * 0.38);
        };

        const eyeRightCenter = () => {
          const p1 = getPt(m[263]), p2 = getPt(m[362]), pTop = getPt(m[386]), pBot = getPt(m[374]);
          if (p1 && p2) return { x: (p1.x + p2.x) / 2, y: (pTop && pBot) ? (pTop.y + pBot.y) / 2 : (p1.y + p2.y) / 2 };
          return pt(473, bx + bw * 0.70, by + bh * 0.38);
        };

        return {
          foreheadTopL: pt(109, bx + bw * 0.30, by + bh * 0.08),
          foreheadTopR: pt(338, bx + bw * 0.70, by + bh * 0.08),
          templeL:      pt(127, bx + bw * 0.12, by + bh * 0.22),
          templeR:      pt(356, bx + bw * 0.88, by + bh * 0.22),
          glabella:     pt(168, bx + bw * 0.50, by + bh * 0.25),
          browMidL:     pt(105, bx + bw * 0.30, by + bh * 0.22),
          browMidR:     pt(334, bx + bw * 0.70, by + bh * 0.22),
          eyeL:         eyeLeftCenter(),
          eyeR:         eyeRightCenter(),
          noseBridge:   pt(6,   bx + bw * 0.50, by + bh * 0.35),
          noseMid:      pt(197, bx + bw * 0.50, by + bh * 0.46),
          noseTip:      pt(4,   bx + bw * 0.50, by + bh * 0.56),
          nostrilL:     pt(98,  bx + bw * 0.40, by + bh * 0.56),
          nostrilR:     pt(327, bx + bw * 0.60, by + bh * 0.56),
          cheekUpperL:  pt(116, bx + bw * 0.16, by + bh * 0.44),
          cheekUpperR:  pt(345, bx + bw * 0.84, by + bh * 0.44),
          cheekLowerL:  pt(147, bx + bw * 0.16, by + bh * 0.64),
          cheekLowerR:  pt(376, bx + bw * 0.84, by + bh * 0.64),
          philtrum:     pt(2,   bx + bw * 0.50, by + bh * 0.68),
          mouthL:       pt(61,  bx + bw * 0.32, by + bh * 0.76),
          mouthR:       pt(291, bx + bw * 0.68, by + bh * 0.76),
          lipBot:       pt(17,  bx + bw * 0.50, by + bh * 0.84),
          chinL:        pt(172, bx + bw * 0.24, by + bh * 0.90),
          chinR:        pt(397, bx + bw * 0.76, by + bh * 0.90),
          chinTip:      pt(152, bx + bw * 0.50, by + bh * 0.98)
        };
      }
      ...
      if (Array.isArray(landmarks68) && landmarks68.length >= 68) {
        ...
        return {
          ...
          nostrilL:     pt(31, bx + bw * 0.40, by + bh * 0.56),
          nostrilR:     pt(35, bx + bw * 0.60, by + bh * 0.56),
      ...
      // 4. Fallback: Exact Canonical Anatomical Proportions
      return {
        foreheadTopL: { x: bx + bw * 0.30, y: by + bh * 0.08 },
        foreheadTopR: { x: bx + bw * 0.70, y: by + bh * 0.08 },
        templeL:      { x: bx + bw * 0.12, y: by + bh * 0.22 },
        templeR:      { x: bx + bw * 0.88, y: by + bh * 0.22 },
        glabella:     { x: bx + bw * 0.50, y: by + bh * 0.25 },
        browMidL:     { x: bx + bw * 0.30, y: by + bh * 0.22 },
        browMidR:     { x: bx + bw * 0.70, y: by + bh * 0.22 },
        eyeL:         { x: bx + bw * 0.30, y: by + bh * 0.38 },
        eyeR:         { x: bx + bw * 0.70, y: by + bh * 0.38 },
        noseBridge:   { x: bx + bw * 0.50, y: by + bh * 0.35 },
        noseMid:      { x: bx + bw * 0.50, y: by + bh * 0.46 },
        noseTip:      { x: bx + bw * 0.50, y: by + bh * 0.56 },
        nostrilL:     { x: bx + bw * 0.40, y: by + bh * 0.56 },
        nostrilR:     { x: bx + bw * 0.60, y: by + bh * 0.56 },
        cheekUpperL:  { x: bx + bw * 0.16, y: by + bh * 0.44 },
        cheekUpperR:  { x: bx + bw * 0.84, y: by + bh * 0.44 },
        cheekLowerL:  { x: bx + bw * 0.16, y: by + bh * 0.64 },
        cheekLowerR:  { x: bx + bw * 0.84, y: by + bh * 0.64 },
        philtrum:     { x: bx + bw * 0.50, y: by + bh * 0.68 },
        mouthL:       { x: bx + bw * 0.32, y: by + bh * 0.76 },
        mouthR:       { x: bx + bw * 0.68, y: by + bh * 0.76 },
        lipBot:       { x: bx + bw * 0.50, y: by + bh * 0.84 },
        chinL:        { x: bx + bw * 0.24, y: by + bh * 0.90 },
        chinR:        { x: bx + bw * 0.76, y: by + bh * 0.90 },
        chinTip:      { x: bx + bw * 0.50, y: by + bh * 0.98 }
      };
    }

    function extract17BiometricLandmarks(landmarks68, bx, by, bw, bh, mesh468 = null) {
      const nodes = resolveBiometricMeshNodes(landmarks68, mesh468, bx, by, bw, bh);
      return [
        nodes.foreheadTopL,
        nodes.foreheadTopR,
        nodes.templeL,
    ...
    // High-Resolution Live Face Snapshot Extractor from Video/Canvas
    function captureLiveFaceSnapshot(box) {
      try {
        const video = document.getElementById('ai-video-player');
        if (video && (video.readyState >= 2 || video.srcObject)) {
          ...
          if (box) {
            bx = (box.x <= 1.05) ? box.x * vw : box.x;
            by = (box.y <= 1.05) ? box.y * vh : box.y;
            bw = (box.width <= 1.05) ? box.width * vw : box.width;
            bh = (box.height <= 1.05) ? box.height * vh : box.height;
          }

          const padX = Math.round(bw * 0.28);
          const padY = Math.round(bh * 0.28);
          const cropX = Math.max(0, Math.round(bx - padX));
          const cropY = Math.max(0, Math.round(by - padY));
          const cropW = Math.min(vw - cropX, Math.round(bw + padX * 2));
          const cropH = Math.min(vh - cropY, Math.round(bh + padY * 2));

          const cropCanvas = document.createElement('canvas');
          cropCanvas.width = 140;
          cropCanvas.height = 168;
          const cCtx = cropCanvas.getContext('2d');
          cCtx.drawImage(snapCanvas, cropX, cropY, cropW, cropH, 0, 0, 140, 168);
    ...
    let _lastVoiceAnnouncementTime = 0;
    let _lastVoiceAnnouncementText = '';
    function speakVoiceAnnouncement(text) {
      if (!isAISoundEnabled) return;
      if (!('speechSynthesis' in window)) return;
      const now = Date.now();
      if (now - _lastVoiceAnnouncementTime < 10000 && _lastVoiceAnnouncementText === text) return;
      _lastVoiceAnnouncementTime = now;
      _lastVoiceAnnouncementText = text;
      try {
    ...
    function triggerAutoLogFace(ent, rawBox) {
      ...
      const isIntervalPassed = !window._lastAutoLogTime || (now - window._lastAutoLogTime > 30000);

      if (!isNewPerson && !isIntervalPassed) return;
      ...
      let snapUrl = ent.snapshot || '';
      if (!snapUrl && rawBox) {
        snapUrl = captureLiveFaceSnapshot(rawBox);
      }
      if (!snapUrl) {
        snapUrl = '../assets/image/avatar-default.png';
      }

      const registeredPhoto = (isKnown && ent.face && ent.face.photo) ? ent.face.photo : '';
      ...
      fd.append('details', isKnown ? `${(ent.face && ent.face.role_title) ? ent.face.role_title : 'Staff'} • Whitelist Verified` : 'Stranger • Pengunjung Belum Terdaftar');
      fd.append('timestamp', getLocalLogTimestamp());

      fetch('../api/ai_analytics.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(data => {
          loadAIData(true);
          if (isKnown) {
            const role = (ent.face && ent.face.role_title) ? ent.face.role_title : (ent.category === 'vip' ? 'VIP' : 'Staff');
            showAIBanner(`${ent.label} (${role})`, `Scan 100% Lock • Similarity: ${ent.confidence}%`, ent.category === 'vip' ? 'badge-success' : 'badge-primary', 'WHITELIST VERIFIED', 'fas fa-user-check', '#059669');
            speakVoiceAnnouncement(`Wajah terdeteksi: ${ent.label}, ${role}`);
          } else {
            showAIBanner(`STRANGER DETECTED`, `Pengunjung Terdeteksi (Scan 100% Lock • ${ent.confidence}%)`, 'badge-warning', 'STRANGER', 'fas fa-user-clock', '#d97706');
          }
        })
        .catch(e => {});
    }
    ...
    function startAIHUDLoop() {
      ...
      function loop() {
        ...
        if (isAutoTrackingActive) {
          const isWebcamRunning = Boolean(video && (video.srcObject !== null || (currentAICamera && currentAICamera.id === 'webcam')));
          const maxStaleMs = isWebcamRunning ? 15000 : 5000;
          if (lastFaceAPIResult && lastFaceAPIResult.faces.length > 0 && (now - lastFaceAPIResult.timestamp < maxStaleMs)) {
            const renderBox = getVideoRenderBox(video, canvas.width, canvas.height);

            const targetEntities = lastFaceAPIResult.faces.map(f => {
              const nb = f.normBox || { x: 0.35, y: 0.30, width: 0.30, height: 0.40 };
              ...
              if (f.normLandmarks && Array.isArray(f.normLandmarks)) {
                scaledLandmarks = f.normLandmarks.map(p => {
                  const px = (p && typeof p.x === 'number' && !isNaN(p.x)) ? p.x : ((p && typeof p._x === 'number' && !isNaN(p._x)) ? p._x : 0.5);
                  const py = (p && typeof p.y === 'number' && !isNaN(p.y)) ? p.y : ((p && typeof p._y === 'number' && !isNaN(p._y)) ? p._y : 0.5);
                  return {
                    x: Math.round(renderBox.x + px * renderBox.width),
                    y: Math.round(renderBox.y + py * renderBox.height)
                  };
                });
              }
              ...
              if (f.mesh468 && Array.isArray(f.mesh468)) {
                scaledMesh468 = f.mesh468.map(p => {
                  const rawX = (p && typeof p.x === 'number' && !isNaN(p.x)) ? p.x : 0.5;
                  const rawY = (p && typeof p.y === 'number' && !isNaN(p.y)) ? p.y : 0.5;
                  const normX = (rawX <= 1.05) ? rawX : rawX / (video && video.videoWidth ? video.videoWidth : 1280);
                  const normY = (rawY <= 1.05) ? rawY : rawY / (video && video.videoHeight ? video.videoHeight : 720);
                  return {
                    x: Math.round(renderBox.x + normX * renderBox.width),
                    y: Math.round(renderBox.y + normY * renderBox.height),
                    z: p.z || 0
                  };
                });
              }
              ...
              if (ptsToCheck && ptsToCheck.length > 0) {
                ...
                // If using 68 landmarks, extend lMinY to include the forehead!
                if ((!scaledMesh468 || scaledMesh468.length < 468) && scaledLandmarks && scaledLandmarks.length >= 68) {
                  const chinPt = scaledLandmarks[8];
                  const browLPt = scaledLandmarks[19];
                  const browRPt = scaledLandmarks[24];
                  if (chinPt && browLPt && browRPt) {
                    const chinY = chinPt.y;
                    const browMidY = (browLPt.y + browRPt.y) / 2;
                    const faceH = Math.max(30, chinY - browMidY);
                    const foreheadY = browMidY - faceH * 0.36;
                    if (foreheadY < lMinY) lMinY = foreheadY;
                  }
                }
              }
              ...
              const targetMeshNodes = resolveBiometricMeshNodes(scaledLandmarks, scaledMesh468, targetX, targetY, targetW, targetH);
              ...
              return {
                ...
                mesh468: scaledMesh468,
                targetMeshNodes: targetMeshNodes,
            ...
            activeAIEntities = targetEntities.map((t) => {
              let bestPrev = null;
              // Expand spatial match radius (75% canvas width) so rapid left/right turns NEVER drop the face
              let bestDist = Math.max(380, canvas.width * 0.75);
              for (const prev of activeAIEntities) {
                if (prev && typeof prev.x === 'number' && isFinite(prev.x) && !isNaN(prev.x)) {
              ...
              const firstSeen = bestPrev ? (bestPrev.firstSeen || (now - 800)) : now;
              const elapsed = now - firstSeen;
              const scanProgress = Math.min(100, Math.floor(20 + (elapsed / 1000) * 80));
              const hasLogged = bestPrev ? (bestPrev.hasLogged || false) : false;
              ...
              const entObj = {
                ...
                hasLogged: hasLogged,
                createdAt: now,
                x: (bestPrev && typeof bestPrev.x === 'number' && isFinite(bestPrev.x) && !isNaN(bestPrev.x)) ? bestPrev.x : t.targetX,
                y: (bestPrev && typeof bestPrev.y === 'number' && isFinite(bestPrev.y) && !isNaN(bestPrev.y)) ? bestPrev.y : t.targetY,
                w: (bestPrev && typeof bestPrev.w === 'number' && isFinite(bestPrev.w) && !isNaN(bestPrev.w) && bestPrev.w > 20) ? bestPrev.w : t.targetW,
                h: (bestPrev && typeof bestPrev.h === 'number' && isFinite(bestPrev.h) && !isNaN(bestPrev.h) && bestPrev.h > 20) ? bestPrev.h : t.targetH,
                currentMeshNodes: (bestPrev && bestPrev.currentMeshNodes)
                  ? bestPrev.currentMeshNodes
                  : { ...t.targetMeshNodes },
                currentLandmarks17: (bestPrev && Array.isArray(bestPrev.currentLandmarks17) && bestPrev.currentLandmarks17.length === 17 && bestPrev.currentLandmarks17.every(p => p && isFinite(p.x) && isFinite(p.y)))
            ...
            // Smooth zero-flicker persistence: keep active tracked entities alive
            const persistLimit = isWebcamRunning ? 15000 : 3500;
            activeAIEntities = activeAIEntities.filter(e => now - (e.createdAt || now) < persistLimit);
          }
        } else {
        ...
        activeAIEntities.forEach(ent => {
          ...
          // Ultra-smooth 60 FPS LERP for all 25 canonical physical facial nodes
          if (ent.targetMeshNodes && ent.currentMeshNodes) {
            const canvasArea = (canvas.width || 640) * (canvas.height || 480);
            const faceArea = (ent.targetW || 1) * (ent.targetH || 1);
            const faceRatio = Math.min(faceArea / canvasArea, 1);
            const lmBaseFactor = 0.88 + Math.min(faceRatio * 0.225, 0.09);

            for (const k in ent.targetMeshNodes) {
              const cur = ent.currentMeshNodes[k];
              const tgt = ent.targetMeshNodes[k];
              if (tgt && typeof tgt.x === 'number' && isFinite(tgt.x) && isFinite(tgt.y)) {
                if (!cur || typeof cur.x !== 'number' || isNaN(cur.x) || !isFinite(cur.x) || typeof cur.y !== 'number' || isNaN(cur.y) || !isFinite(cur.y)) {
                  ent.currentMeshNodes[k] = { x: tgt.x, y: tgt.y };
                } else {
                  const ptDist = Math.hypot(tgt.x - cur.x, tgt.y - cur.y);
                  const ptBoost = Math.min(ptDist / 80, 0.08);
                  const lmFactor = Math.min(lmBaseFactor + ptBoost, 0.98);
        ...
        // Bottom OSD with real-time biometric database status & live scan distance
        ctx.font = '600 11px sans-serif';
        ctx.fillStyle = 'rgba(255, 255, 255, 0.45)';
        const descCount = (window.allRegisteredDescriptors && window.allRegisteredDescriptors.length > 0 ? window.allRegisteredDescriptors.length : 0) || (Array.isArray(cachedAIFaces) ? cachedAIFaces.length : 0);
        const dbStatus = descCount > 0 ? `DB: ${descCount} IDENTITY ACTIVE` : 'DB: INITIALIZING BIOMETRICS...';
        let debugMatch = '';
        if (window._lastLiveBiometricDebug && (Date.now() - window._lastLiveBiometricDebug.timestamp < 3000)) {
          const d = window._lastLiveBiometricDebug;
          if (d.bestCandidate) {
            debugMatch = ` • SCAN: ${d.bestCandidate.toUpperCase()} (D:${d.bestDist.toFixed(2)})`;
          }
        }
        ctx.fillText(`PT. LOEWIX INDONESIA • AI NEURAL BIOMETRIC ENGINE V4.0 • ${dbStatus}${debugMatch} • 60 FPS LERP`, 18, canvas.height - 16);
    ...
    /**
     * Generates a dense 3D Biometric Constellation Cloud (~160-220 glowing cyan points)
     * strategically mapped to facial anatomy (forehead, brows, orbits, nose, cheeks, mouth, jaw).
     * Replicates the exact visual appearance of MediaPipe 468 3D FaceMesh in Gambar 2.
     */
    function generateBiometricFaceCloud(pts, x, y, w, h) {
      try {
        const cloud = [];
        const addPt = (px, py) => {
          if (typeof px === 'number' && typeof py === 'number' && !isNaN(px) && !isNaN(py) && isFinite(px) && isFinite(py)) {
            cloud.push({ x: px, y: py });
          }
        };

        const safeP = (p, fbX, fbY) => {
          if (p && typeof p.x === 'number' && typeof p.y === 'number' && !isNaN(p.x) && !isNaN(p.y) && isFinite(p.x) && isFinite(p.y)) {
            return p;
          }
          return {
            x: (typeof fbX === 'number' && !isNaN(fbX) && isFinite(fbX)) ? fbX : (x + w * 0.5),
            y: (typeof fbY === 'number' && !isNaN(fbY) && isFinite(fbY)) ? fbY : (y + h * 0.5)
          };
        };

        const lerpPt = (p1, p2, t) => {
          const a = safeP(p1, x + w * 0.5, y + h * 0.5);
          const b = safeP(p2, x + w * 0.5, y + h * 0.5);
          return {
            x: a.x + (b.x - a.x) * t,
            y: a.y + (b.y - a.y) * t
          };
        ...
        for (let i = 1; i < 6; i++) {
          const t = i / 6;
          const ptL = lerpPt(pts.templeL, glab, t);
          addPt(ptL.x, ptL.y - (h * 0.01));
          const ptR = lerpPt(glab, pts.templeR, t);
          addPt(ptR.x, ptR.y - (h * 0.01));
        }

        // 3. Eye orbital rings (8 points around each eye)
        const eyeL = safeP(pts.eyeL, x + w * 0.30, y + h * 0.36);
        const eyeR = safeP(pts.eyeR, x + w * 0.70, y + h * 0.36);
        const eyeRadX = w * 0.09;
        const eyeRadY = h * 0.06;
        for (let a = 0; a < 8; a++) {
          const rad = (a / 8) * Math.PI * 2;
          addPt(eyeL.x + Math.cos(rad) * eyeRadX, eyeL.y + Math.sin(rad) * eyeRadY);
          addPt(eyeR.x + Math.cos(rad) * eyeRadX, eyeR.y + Math.sin(rad) * eyeRadY);
        }

        // 4. Glabella & Bridge Matrix (6 points)
        const bridge = safeP(pts.noseBridge, x + w * 0.50, y + h * 0.34);
        for (let i = 1; i <= 3; i++) {
          const t = i / 4;
          const center = lerpPt(glab, bridge, t);
          addPt(center.x - w * 0.03, center.y);
          addPt(center.x + w * 0.03, center.y);
        }

        // 5. Nose ridge, sidewalls, and tip (14 points)
        const noseTip = safeP(pts.noseTip, x + w * 0.50, y + h * 0.58);
        const nostrilL = safeP(pts.nostrilL, x + w * 0.40, y + h * 0.58);
        const nostrilR = safeP(pts.nostrilR, x + w * 0.60, y + h * 0.58);

        for (let i = 1; i <= 4; i++) {
          const t = i / 5;
          const pMid = lerpPt(bridge, noseTip, t);
          addPt(pMid.x, pMid.y);
          addPt(lerpPt(bridge, nostrilL, t).x, lerpPt(bridge, nostrilL, t).y);
          addPt(lerpPt(bridge, nostrilR, t).x, lerpPt(bridge, nostrilR, t).y);
        }
        addPt(lerpPt(nostrilL, noseTip, 0.5).x, lerpPt(nostrilL, noseTip, 0.5).y);
        addPt(lerpPt(nostrilR, noseTip, 0.5).x, lerpPt(nostrilR, noseTip, 0.5).y);

        // 6. Dense Cheek Surface Matrices (25 points left cheek, 25 points right cheek = 50 points)
        for (let row = 1; row <= 5; row++) {
          const tr = row / 6;
          const leftBoundary = lerpPt(pts.templeL, pts.cheekLowerL, tr);
          const leftInner    = lerpPt(glab, pts.mouthL, tr);
          for (let col = 1; col <= 5; col++) {
            const tc = col / 6;
            const cp = lerpPt(leftBoundary, leftInner, tc);
            addPt(cp.x, cp.y + (Math.sin(tc * Math.PI) * (h * 0.015)));
          }

          const rightInner    = lerpPt(glab, pts.mouthR, tr);
          const rightBoundary = lerpPt(pts.templeR, pts.cheekLowerR, tr);
          for (let col = 1; col <= 5; col++) {
            const tc = col / 6;
            const cp = lerpPt(rightInner, rightBoundary, tc);
            addPt(cp.x, cp.y + (Math.sin(tc * Math.PI) * (h * 0.015)));
          }
        }

        // 7. Philtrum & Mouth Matrix (15 points)
        const phil = safeP(pts.philtrum, x + w * 0.50, y + h * 0.68);
        const mouthL = safeP(pts.mouthL, x + w * 0.32, y + h * 0.78);
        const mouthR = safeP(pts.mouthR, x + w * 0.68, y + h * 0.78);
        const lipBot = safeP(pts.lipBot, x + w * 0.50, y + h * 0.86);

        for (let i = 1; i < 7; i++) {
          const t = i / 7;
          const pUpper = (t < 0.5) ? lerpPt(mouthL, phil, t * 2) : lerpPt(phil, mouthR, (t - 0.5) * 2);
          addPt(pUpper.x, pUpper.y);
        }
        for (let i = 1; i < 7; i++) {
          const t = i / 7;
          const pLower = (t < 0.5) ? lerpPt(mouthL, lipBot, t * 2) : lerpPt(lipBot, mouthR, (t - 0.5) * 2);
          addPt(pLower.x, pLower.y);
        }
        for (let i = 1; i <= 3; i++) {
          const t = i / 4;
          addPt(lerpPt(mouthL, mouthR, t).x, lerpPt(mouthL, mouthR, t).y);
        }
        ...
        for (let r = 1; r <= 3; r++) {
          const tr = r / 4;
          const cL = lerpPt(chinL, chinTip, tr);
          const cR = lerpPt(chinR, chinTip, tr);
          for (let c = 1; c <= 4; c++) {
            const tc = c / 5;
            addPt(lerpPt(cL, cR, tc).x, lerpPt(cL, cR, tc).y);
          }
        }
        for (let i = 1; i < 6; i++) {
          const t = i / 6;
          addPt(lerpPt(chinL, chinTip, t).x, lerpPt(chinL, chinTip, t).y);
          addPt(lerpPt(chinR, chinTip, t).x, lerpPt(chinR, chinTip, t).y);
        }

        return cloud;
      } catch (errCloud) {
        console.warn('[Cloud Generate Error]', errCloud);
        return [];
      }
    }
     ...
     * 6. Dense 3D Constellation Cloud (~180 glowing cyan points across face)
     * 7. High-contrast commercial VMS identification badge with neon lime accents
     */
    function drawBiometricFacialMesh(ctx, x, y, w, h, ent, isUnknown, isVIP, isBlacklist) {
      ...
      try {
        ...
        const safePt = (p, fbX, fbY) => {
          const pos = ptPos(p);
          if (pos) return pos;
          return {
            x: isValidNum(fbX) ? fbX : (x + w * 0.5),
            y: isValidNum(fbY) ? fbY : (y + h * 0.5)
          };
        };

        // Continuous Responsive Biometric Mesh Solver:
        // Uses 60 FPS interpolated physical facial nodes from ent.currentMeshNodes,
        // dynamically wrapping the real contours of the face across head yaw, pitch, and roll!
        const rawPts = (ent.currentMeshNodes && ent.currentMeshNodes.chinTip)
          ? ent.currentMeshNodes
          : resolveBiometricMeshNodes(ent.landmarks, ent.mesh468, x, y, w, h);

        const pts = {
          foreheadTopL: safePt(rawPts.foreheadTopL, x + w * 0.30, y + h * 0.08),
          foreheadTopR: safePt(rawPts.foreheadTopR, x + w * 0.70, y + h * 0.08),
          templeL:      safePt(rawPts.templeL,      x + w * 0.12, y + h * 0.22),
          templeR:      safePt(rawPts.templeR,      x + w * 0.88, y + h * 0.22),
          glabella:     safePt(rawPts.glabella,     x + w * 0.50, y + h * 0.25),

          browMidL:     safePt(rawPts.browMidL,     x + w * 0.30, y + h * 0.22),
          browMidR:     safePt(rawPts.browMidR,     x + w * 0.70, y + h * 0.22),

          eyeL:         safePt(rawPts.eyeL,         x + w * 0.30, y + h * 0.38),
          eyeR:         safePt(rawPts.eyeR,         x + w * 0.70, y + h * 0.38),

          noseBridge:   safePt(rawPts.noseBridge,   x + w * 0.50, y + h * 0.35),
          noseMid:      safePt(rawPts.noseMid,      x + w * 0.50, y + h * 0.46),
          noseTip:      safePt(rawPts.noseTip,      x + w * 0.50, y + h * 0.56),
          nostrilL:     safePt(rawPts.nostrilL,     x + w * 0.40, y + h * 0.56),
          nostrilR:     safePt(rawPts.nostrilR,     x + w * 0.60, y + h * 0.56),

          cheekUpperL:  safePt(rawPts.cheekUpperL,  x + w * 0.16, y + h * 0.44),
          cheekUpperR:  safePt(rawPts.cheekUpperR,  x + w * 0.84, y + h * 0.44),
          cheekLowerL:  safePt(rawPts.cheekLowerL,  x + w * 0.16, y + h * 0.64),
          cheekLowerR:  safePt(rawPts.cheekLowerR,  x + w * 0.84, y + h * 0.64),

          philtrum:     safePt(rawPts.philtrum,     x + w * 0.50, y + h * 0.68),
          mouthL:       safePt(rawPts.mouthL,       x + w * 0.32, y + h * 0.76),
          mouthR:       safePt(rawPts.mouthR,       x + w * 0.68, y + h * 0.76),
          lipBot:       safePt(rawPts.lipBot,       x + w * 0.50, y + h * 0.84),

          chinL:        safePt(rawPts.chinL,        x + w * 0.24, y + h * 0.90),
          chinR:        safePt(rawPts.chinR,        x + w * 0.76, y + h * 0.90),
          chinTip:      safePt(rawPts.chinTip,      x + w * 0.50, y + h * 0.98)
        };
        ...
        const edges = [
          ...
          [pts.foreheadTopL, pts.templeL],
          [pts.foreheadTopR, pts.templeR],
          ...
          [pts.templeR, pts.cheekUpperR],

          // Eyebrows to glabella & eyes
          [pts.browMidL, pts.glabella],
          [pts.browMidR, pts.glabella],
          [pts.browMidL, pts.eyeL],
          [pts.browMidR, pts.eyeR],
          [pts.glabella, pts.eyeL],
          [pts.glabella, pts.eyeR],

          // Glabella & eyes to nose bridge & cheekbones
          [pts.glabella,    pts.noseBridge],
          [pts.eyeL,        pts.noseBridge],
          [pts.eyeR,        pts.noseBridge],
          ...
          [pts.nostrilR,   pts.noseTip],
          [pts.nostrilL,   pts.nostrilR],
          ...
          [pts.cheekUpperL, pts.mouthL],
          [pts.cheekUpperR, pts.mouthR],
          ...
          [pts.lipBot,  pts.chinR],
          [pts.lipBot,  pts.chinTip],
          [pts.chinL,   pts.chinTip],
          [pts.chinR,   pts.chinTip]
        ];
        ...
        ctx.beginPath();
        edges.forEach(([p1, p2]) => {
          if (p1 && p2 && isValidNum(p1.x) && isValidNum(p1.y) && isValidNum(p2.x) && isValidNum(p2.y)) {
        ...
        // 2B. 3D Constellation Cloud (Dense Glowing Cyan Points from Gambar 2)
        // Use raw MediaPipe 468 if available, or generate anatomically mapped constellation cloud
        const cloud = (Array.isArray(ent.mesh468) && ent.mesh468.length > 0)
          ? ent.mesh468
          : generateBiometricFaceCloud(pts, x, y, w, h);
        ...
        if (Array.isArray(cloud) && cloud.length > 0) {
          ...
          ctx.shadowColor = '#00f0ff';
          ctx.shadowBlur = 4;
          const step = (cloud.length > 300) ? 2 : 1;
          for (let i = 0; i < cloud.length; i += step) {
            const mp = cloud[i];
            if (mp && isValidNum(mp.x) && isValidNum(mp.y)) {
              ctx.beginPath();
              ctx.arc(mp.x, mp.y, 1.3, 0, Math.PI * 2);
              ctx.fill();
            }
          }
          ctx.restore();
        ...
        const allNodes = Object.values(pts).filter(p => p && isValidNum(p.x) && isValidNum(p.y));
        ...
        allNodes.forEach((pt, idx) => {
          const pulse = Math.sin(pulseTime + idx * 0.4) * 0.9;
          ctx.beginPath();
          ctx.arc(pt.x, pt.y, Math.max(6.5, 9.0 + pulse), 0, Math.PI * 2);
          ctx.fill();
        });

        // Layer B: Cyan energy halo
        ctx.shadowColor = '#00f0ff';
        ctx.shadowBlur = 12;
        ctx.fillStyle = 'rgba(0, 240, 255, 0.70)';
        allNodes.forEach(pt => {
          ctx.beginPath();
          ctx.arc(pt.x, pt.y, 5.8, 0, Math.PI * 2);
          ctx.fill();
        });

        // Layer C: Medium bright luminous white halo
        ctx.shadowColor = '#ffffff';
        ctx.shadowBlur = 8;
        ctx.fillStyle = 'rgba(255, 255, 255, 0.95)';
        allNodes.forEach(pt => {
          ctx.beginPath();
          ctx.arc(pt.x, pt.y, 4.8, 0, Math.PI * 2);
          ctx.fill();
        });

        // Layer D: Solid gleaming white star core
        ctx.shadowBlur = 4;
        ...
        console.error('[Biometric Facial Mesh Exception]', errMeshMaster);
      } finally {
        ctx.restore();
      }
    }
    ...
    function drawEntityBracket(ctx, ent) {
      ...
      const cW = (ctx.canvas && ctx.canvas.width) ? ctx.canvas.width : 640;
      const cH = (ctx.canvas && ctx.canvas.height) ? ctx.canvas.height : 380;
      let x = Number(ent.x);
      let y = Number(ent.y);
      let w = Number(ent.w);
      let h = Number(ent.h);
      if (!isFinite(w) || isNaN(w) || w < 40) w = Math.round(cW * 0.30);
      if (!isFinite(h) || isNaN(h) || h < 40) h = Math.round(w * 1.25);
      if (!isFinite(x) || isNaN(x)) x = Math.round((cW - w) / 2);
      if (!isFinite(y) || isNaN(y)) y = Math.round(cH * 0.22);
      ...
      let bracketColor = isScanning ? '#00f0ff' : (isBlacklist ? '#ef4444' : (isVIP ? '#ccff00' : (isRealStranger ? '#ffd700' : '#00f0ff')));
      let bracketGlow = isScanning ? 'rgba(0, 240, 255, 0.95)' : (isBlacklist ? 'rgba(239, 68, 68, 0.90)' : (isVIP ? 'rgba(204, 255, 0, 0.95)' : (isRealStranger ? 'rgba(255, 215, 0, 0.90)' : 'rgba(0, 240, 255, 0.95)')));
      ...
      // Scanning HUD Line inside bracket while scanning
      if (isScanning) {
      ...
      // 3. Commercial VMS Identification / Scanning Tag
      let fullTagText, confStr, pillColor;
      if (isScanning) {
        fullTagText = `⚡ SCANNING BIOMETRIC [${scanProgress}%]`;
        confStr = `${scanProgress}%`;
        pillColor = '#00f0ff';
      } else {
        const isStranger = isRealStranger;
        ...
        if (isStranger) {
          ...
          roleTag = ' [DPO BLACKLIST]';
        } else if (category === 'guest') {
          roleTag = ' [TAMU TERDAFTAR]';
      ...
      const tagX = Math.max(4, Math.min(canvasW - tagW - 4, bx + (bw - tagW) / 2));
      let tagY = by - tagH - 8;
      if (tagY < 4) tagY = by + 8; // clamp inside if touching top canvas ceiling

      // Solid High-Contrast Cyber Tag Box
      ...
      // Label Name + Role / Scanning
      ctx.fillStyle = '#ffffff';
      ctx.font = '800 12.5px "Plus Jakarta Sans", -apple-system, BlinkMacSystemFont, sans-serif';
      ctx.fillText(fullTagText, tagX + 24, tagY + 18.5);

      // Confidence Pill
      ctx.fillStyle = pillColor;
      ctx.font = '800 11.5px monospace';
      ctx.textAlign = 'right';
      ctx.fillText(confStr, tagX + tagW - 10, tagY + 18.5);
      ctx.textAlign = 'left';
    ...
    async function changeAICamera(camId) {
      ...
      if (camId === 'webcam') {
        ...
        currentAICamera = { id: 'webcam', title: 'Live Webcam Laptop' };
        if (statusLabel) {
          statusLabel.innerHTML = '<span class="text-emerald" style="color: #34d399;"><i class="fas fa-video mr-1"></i> Live Webcam Scanner Aktif (Auto Detect)</span>';
        }
        startAIWebcamLive();
      } else {
        if (aiMainWebcamStream) {
    ...
    function simulateCustomFaceDetection(name, category = 'employee', roleTitle = 'Staff') {
      ...
      const rawFacePhoto = faceObj ? faceObj.photo : '';
      const photoSrc = rawFacePhoto ? resolveFacePhotoUrl(rawFacePhoto) : '../assets/image/avatar-default.png';
      fd.append('action', 'log_detection');
      fd.append('type', 'face');
      fd.append('camera_id', activeCamId);
      fd.append('camera_title', activeCamTitle);
    ...
    async function submitRegisterFace(e) {
      ...
      const photoInput = document.getElementById('face-input-photo');
      let photoVal = photoInput ? photoInput.value : '';

      // Fallback: If hidden input photo is empty, check face-preview-img src
      if (!photoVal) {
        const pImg = document.getElementById('face-preview-img');
        if (pImg && pImg.src && (pImg.src.startsWith('data:image') || pImg.src.startsWith('http') || pImg.src.startsWith('assets/'))) {
          photoVal = pImg.src;
          if (photoInput) photoInput.value = photoVal;
        }
      }

      if (!photoVal || (!photoVal.startsWith('data:image') && !photoVal.startsWith('http') && !photoVal.startsWith('assets/'))) {
        alert('⚠️ Wajib sertakan foto wajah terlebih dahulu (bisa lewat scan webcam, upload galeri, atau snapshot CCTV)!');
        return;
      }

      // Instant UI response: show loading spinner immediately
      if (btnSubmit) {
      ...
      try {
        ...
        fd.append('category', document.getElementById('face-input-category')?.value || 'employee');
        fd.append('role_title', document.getElementById('face-input-role')?.value || 'Staff');
        ...
        try {
          ...
          const cleanText = firstBrace !== -1 ? text.substring(firstBrace) : text;
          data = JSON.parse(cleanText);
        } catch(eJson) {
          console.error('[Face Register] Raw API response:', text);
        ...
        if (data.success) {
          ...
          if (Array.isArray(parsedDesc) && parsedDesc.length === 128) {
            ...
            allRegisteredDescriptors = allRegisteredDescriptors.filter(ld => ld.label.toLowerCase() !== nameVal.toLowerCase());
        ...
        console.error(err);
        alert('Terjadi kesalahan saat menyimpan data wajah: ' + err.message);
      } finally {
        if (btnSubmit) {
