...
<body>
  ...
  <script>
    ...
    /**
     * High-Tech 3D Biometric Facial Mesh & Glowing White Node Renderer (Faithful to Gambar 1)
     * Renders exact glowing circular white nodes, polygon triangulation mesh, and neon lime framing
     */
    function drawBiometricFacialMesh(ctx, x, y, w, h, ent, isUnknown, isVIP, isBlacklist) {
      ...
      if (has68) {
        ...
        pts = {
          foreheadTopL: { x: l[19].x - (w * 0.04), y: Math.min(l[19].y, l[24].y) - (h * 0.22) },
          foreheadTopR: { x: l[24].x + (w * 0.04), y: Math.min(l[19].y, l[24].y) - (h * 0.22) },
          foreheadMid:  { x: (l[19].x + l[24].x) / 2, y: Math.min(l[19].y, l[24].y) - (h * 0.20) },
          templeL:      { x: l[0].x - (w * 0.03), y: l[17].y - (h * 0.06) },
          templeR:      { x: l[16].x + (w * 0.03), y: l[26].y - (h * 0.06) },
        ...
        // High-Precision Canonical Facial Geometry mapped directly to face bounding box
        pts = {
          foreheadTopL: { x: x + w * 0.28, y: y + h * 0.10 },
          foreheadTopR: { x: x + w * 0.72, y: y + h * 0.10 },
          foreheadMid:  { x: x + w * 0.50, y: y + h * 0.08 },
          templeL:      { x: x + w * 0.12, y: y + h * 0.23 },
          templeR:      { x: x + w * 0.88, y: y + h * 0.23 },
          glabella:     { x: x + w * 0.50, y: y + h * 0.33 },

          browOutL: { x: x + w * 0.22, y: y + h * 0.28 },
          browMidL: { x: x + w * 0.35, y: y + h * 0.26 },
          browMidR: { x: x + w * 0.65, y: y + h * 0.26 },
          browOutR: { x: x + w * 0.78, y: y + h * 0.28 },

          eyeL: { x: x + w * 0.33, y: y + h * 0.38 },
          eyeR: { x: x + w * 0.67, y: y + h * 0.38 },

          noseBridge: { x: x + w * 0.50, y: y + h * 0.44 },
          noseTip:    { x: x + w * 0.50, y: y + h * 0.62 },
          nostrilL:   { x: x + w * 0.39, y: y + h * 0.61 },
          nostrilR:   { x: x + w * 0.61, y: y + h * 0.61 },

          cheekUpperL: { x: x + w * 0.17, y: y + h * 0.47 },
          cheekUpperR: { x: x + w * 0.83, y: y + h * 0.47 },
          cheekLowerL: { x: x + w * 0.16, y: y + h * 0.68 },
          cheekLowerR: { x: x + w * 0.84, y: y + h * 0.68 },

          philtrum: { x: x + w * 0.50, y: y + h * 0.71 },
          mouthL:   { x: x + w * 0.33, y: y + h * 0.78 },
          mouthR:   { x: x + w * 0.67, y: y + h * 0.78 },
          lipBot:   { x: x + w * 0.50, y: y + h * 0.82 },

          chinCornerL: { x: x + w * 0.31, y: y + h * 0.90 },
          chinCornerR: { x: x + w * 0.69, y: y + h * 0.90 },
          chinTip:     { x: x + w * 0.50, y: y + h * 0.99 }
        };
      }

      // Triangulation Network (Matching Gambar 1)
      const edges = [
        // Forehead top bar
        [pts.foreheadTopL, pts.foreheadTopR],

        // Forehead star / triangulation fan
        [pts.foreheadTopL, pts.templeL],
        [pts.foreheadTopR, pts.templeR],
        [pts.foreheadTopL, pts.foreheadMid],
        [pts.foreheadTopR, pts.foreheadMid],
        [pts.foreheadMid,  pts.glabella],
        [pts.foreheadTopL, pts.glabella],
        [pts.foreheadTopR, pts.glabella],
        [pts.templeL,      pts.glabella],
        [pts.templeR,      pts.glabella],

        // Temples to eyebrows & outer cheeks
        ...
        [pts.cheekLowerL, pts.chinCornerL],
        [pts.cheekLowerR, pts.chinCornerR],
        ...
        [pts.chinCornerL, pts.chinTip],
        [pts.chinCornerR, pts.chinTip]
      ];

      ctx.save();

      // 1. Draw Biometric Triangulation Lines (from Gambar 1)
      ...
      // 2. Draw Glowing White Biometric Dots (Exactly as in Gambar 1)
      ...
      // Layer B: Solid gleaming white center core
      ctx.shadowBlur = 8;
      allNodes.forEach(pt => {
        ctx.beginPath();
    ...
    function drawEntityBracket(ctx, ent) {
      ...
      // 1. Draw Biometric Mesh & Glowing White Dots (Gambar 1 style)
      drawBiometricFacialMesh(ctx, x, y, w, h, ent, isUnknown, isVIP, isBlacklist);

      // 2. Thick Neon Lime-Green Corner Brackets (Faithful to Gambar 1)
      const bracketColor = isBlacklist ? '#ef4444' : '#bbf451'; // Electric Neon Lime / Chartreuse
      const bracketGlow = isBlacklist ? 'rgba(239, 68, 68, 0.85)' : 'rgba(187, 244, 81, 0.85)';
      ...
      const bw = w + pad * 2;
      const bh = h + pad * 2;
      const armLen = Math.min(48, Math.max(26, Math.round(bw * 0.23)));
      ...
      // 3. Commercial VMS Identification Tag (High-contrast, bold, crisp above top bracket)
      ...
      let tagY = by - tagH - 6;
      if (tagY < 4) tagY = by + 6; // clamp inside if touching top canvas ceiling
      ...
      // Confidence Pill
      ctx.fillStyle = isStranger ? '#ffd700' : (isBlacklist ? '#ef4444' : '#a3e635');
      ctx.font = '800 11.5px monospace';
      ctx.textAlign = 'right';
