...
<body>
  ...
  <script>
    ...
    function checkLocalStorageSession() {
      ...
      mediamtxData.length = 0;
      if (typeof generateCCTVHTML === 'function') generateCCTVHTML(currentGlobalCity);
    }
    ...
    function renderUserSessionUI(user) {
      ...
      if (user) {
        ...
        userArea.innerHTML = `
          <button class="btn btn-sm" onclick="openLoginModal()" style="background: linear-gradient(135deg, #00d2ff, #0066ff); border: none; font-weight: 700; border-radius: 25px; padding: 7px 20px; color: #fff; box-shadow: 0 4px 15px rgba(0, 102, 255, 0.4); text-transform: uppercase; font-size: 12px; letter-spacing: 0.5px; white-space: nowrap;">
            <i class="fas fa-sign-in-alt mr-1"></i> LOGIN
          </button>
        `;
      }
