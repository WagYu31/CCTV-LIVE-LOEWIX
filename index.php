...
<body>
  ...
  <script nonce="yoE9kVwZ3muQRX/H9UUiLA==">
    ...
    async function saveEncodeConfig() {
      ...
      if (okBtn) {
        okBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Menyimpan...';
        okBtn.disabled = true;
      }
      ...
      try {
        ...
        if (data.success) {
          if (typeof showCCTVToast === 'function') {
            showCCTVToast(data.message || `Konfigurasi Encode Channel ${currentEncodeChannel} berhasil disimpan!`, 'success');
          } else {
            alert(data.message || `Konfigurasi Encode Channel ${currentEncodeChannel} berhasil disimpan!`);
          }
          closeEncodeModal();
        } else {
